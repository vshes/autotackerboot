...
import io.shesh.tracker.dao.AlertRepository;
...
public class AlertServiceImpl implements AlertService{
    ...
    public List<Alert> findAllByVehicle(String id) {
        return alertDao.findByVin(id);
    }

    @Override
    public void delete(String vin) {
        alertDao.deleteByVin(vin);
    }
    ...
    private List<Alert> createAlerts(Readings readings){
        ...
        if((readings.getEngineRpm() > vehicleDao.findOne(readings.getVin()).getRedlineRpm())){
            Alert alert = new Alert();
            alert.setLatitude(readings.getLatitude());
            alert.setLongitude(readings.getLongitude());
        ...
        if((( readings.getFuelVolume() / vehicleDao.findOne(readings.getVin()).getMaxFuelVolume() ) * 100 ) < 10){
            Alert alert = new Alert();
            alert.setLatitude(readings.getLatitude());
            alert.setLongitude(readings.getLongitude());
